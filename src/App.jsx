...
const App = () => {
  ...
  const createChunksGlobal = () => {
    ...
    if (portion === 'verse') {
      ...
      allAyahsInRange.forEach(a => { if (!pageGroups[a.page]) pageGroups[a.page] = []; pageGroups[a.page].push(a); });
      Object.values(pageGroups).forEach(group => {
        const t1 = Math.ceil(group.length / 3);
        const t2 = Math.ceil(2 * group.length / 3);
        newChunks.push(group.slice(0, t1));
        newChunks.push(group.slice(t1, t2));
        newChunks.push(group.slice(t2));
      });
  ...
  const startMusaffa = () => {
    const finalChunks = createChunksGlobal();
    if (finalChunks.length === 0) return;
    currentIndexRef.current = 0;
    setCurrentChunkIndex(0);
    setPartnerSubView('mudarasa');
    if (musaffaParams.whoStarts === 'app') {
  ...
  const startQuiz = () => {
    setCurrentQuizIndex(0); setQuizScore(0); setQuizFeedback(null);
    setPartnerSubView('quiz'); setView('partner');
  }
